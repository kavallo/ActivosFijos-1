...
Public Class ACTMAFC0
    ...
    Private Sub Forma_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        lblMEJORA_CORRELATIVO.Text = CodigoMejora
        lblCODIGO_DE_ACTIVO.Text = CodigoActivo
                    ...
                    For Each filaData As DataRow In dataLabels.Tables(0).Rows
                        ...
                        If listaValores = "N" Then
                            ...
                            With oTexto
                                ...
                                Else
                                    .EsRequerido = True
                                End If
                                '-- Como es sólo de consulta no se puede modificar
                                .Enabled = False
                            End With
                        ...
                        Else
                            ...
                            With oCombo
                                ...
                                '-- Como sólo es de consulta no se puede modificar
                                .Enabled = False
    ...
    Private Sub Leer()
        Try
            ...
            meteSQL(" ltrim(str(ACTIVOS_FIJOS.COSTO_ACUM_MNTMTO,17,9)), ")
            ...
            meteSQL(" where ACTIVOS_FIJOS.CODIGO_DE_ACTIVO = " & Scm(CodigoActivo) & " and ACTIVOS_FIJOS.MEJORA_CORRELATIVO = " & sen(CodigoMejora))

            EjecutaSQL("")

            Do While SqlNextRow(Sqlconn) = MOREROWS
                lblANO_PERIODO.Text = SqlData(Sqlconn, 1).ToString
                lblNO_PERIODO.Text = SqlData(Sqlconn, 2).ToString
                lblSALDO_MESES_DEPRE.Text = SqlData(Sqlconn, 3).ToString
                ...
                lblVALOR_ORIGINAL_ACTIV.Text = SqlData(Sqlconn, 8).ToString
                lblVALOR_ACTUAL_ACTIVO.Text = SqlData(Sqlconn, 9).ToString
                lblVALOR_RESCATE_ACTIVO.Text = SqlData(Sqlconn, 10).ToString
                Me.cboTipoDepreciacion.SelectedValue = SqlData(Sqlconn, 11)
                lblTIEMPO_DEPRECIACION.Text = SqlData(Sqlconn, 12).ToString
                lblPORCENTAJE_ANUAL_DEP.Text = SqlData(Sqlconn, 13).ToString
                lblFECHA_ADQUISICION.Text = SqlData(Sqlconn, 14).ToString
                lblFECHA_BAJA_ACTIVO.Text = SqlData(Sqlconn, 15).ToString
                lblVALOR_DEP_ACUM_ACT.Text = SqlData(Sqlconn, 16).ToString
                lblMONTO_ULTIMA_REVAL.Text = SqlData(Sqlconn, 17).ToString
                lblFECHA_ULTIMA_REVAL.Text = SqlData(Sqlconn, 18).ToString
                lblMONTO_ULTIMA_DEPREC.Text = SqlData(Sqlconn, 19).ToString
                ...
                Me.cboEstado.SelectedValue = SqlData(Sqlconn, 23)
                lblTASA_CAMBIO_COMPRA.Text = SqlData(Sqlconn, 24).ToString
                lblTASA_CAMBIO_ULT_REV.Text = SqlData(Sqlconn, 25).ToString
                ...
                lblCOSTO_ACUM_MNTMTO.Text = SqlData(Sqlconn, 32).ToString
                lblFRECUENCIA_MNTMTO.Text = SqlData(Sqlconn, 33).ToString
                Me.cboDepreciable.SelectedValue = SqlData(Sqlconn, 34)
                Me.cboEstadoServicio.SelectedValue = SqlData(Sqlconn, 35)
                lblVALOR_ORIG_ACT_USADO.Text = SqlData(Sqlconn, 36).ToString
                lblTASA_DEDUCIBLE_ISR.Text = SqlData(Sqlconn, 37).ToString
                lblVALOR_NO_DEDUCIBLE.Text = SqlData(Sqlconn, 38).ToString
                txtREFERENCIA.Text = SqlData(Sqlconn, 39).ToString
                txtCODIGO_MARCA.Text = SqlData(Sqlconn, 40).ToString
                txtVALOR_MONEDA_LOCAL.Text = SqlData(Sqlconn, 41).ToString
            ...
            '***********************
            'LEEMOS VALORES DE LOS CAMPOS CUSTOMIZADOS
            '***********************
            Me.LeeCampos()
            ...
            meteSQL(" where M.CODIGO_DE_ACTIVO   = " & Scm(CodigoActivo))
            meteSQL("   and M.MEJORA_CORRELATIVO = " & sen(CodigoMejora))
            ...
            If lblVALOR_ORIGINAL_ACTIV.Text = "" Then
                lblVALOR_ORIGINAL_ACTIV.Text = CStr(0)
            End If
            ...
            Do While SqlNextRow(Sqlconn) = MOREROWS
                lblVALOR_DEP_PERIODO.Text = SqlData(Sqlconn, 1).ToString
            Loop

            If lblSALDO_MESES_DEPRE.Text = "" Then
                lblSALDO_MESES_DEPRE.Text = "0"
            End If
    ...
    Private Sub InfoGeneral_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdInfoGeneral.Click
        If Not String.IsNullOrEmpty(TraeDato("select count(*) from ACTIVOS_FIJOS_GEN where CODIGO_DE_ACTIVO = " & Scm(CodigoActivo) & " and MEJORA_CORRELATIVO = " & sen(CodigoMejora))) Then
            If Val(TraeDato("select count(*) from ACTIVOS_FIJOS_GEN where CODIGO_DE_ACTIVO = " & Scm(CodigoActivo) & " and MEJORA_CORRELATIVO = " & sen(CodigoMejora)).ToString) > 0 Then
                Dim frmConsulta As New ACTAFGC0
                With frmConsulta
                    .Modo_Local = Modo_Consulta
                    .SoloDeConsulta = True
                    .CodigoActivo = Me.CodigoActivo
                    .CodigoMejora = Me.CodigoMejora
                    .ShowDialog()
                    .Dispose()
                End With
            Else
